...
def test_invalid_max_iter():
    ...
    try:
      ...
      assert False,  "Problem: no error when max_iter=0"
...
def test_invalid_mat_dim():
     ...
     # Create random 
     test_mat = np.random.randn(10,3,3)

     try:
       kmeans_test_model = km.KMeans(k=2)
       kmeans_test_model.fit(mat = test_mat)
       
       assert False, "3D matrix for mat as input for fit method should have raised an error"
    
     except ValueError as e:
       assert str(e) == "Provided mat should be a 2D matrix",  "3D matrix for mat as input for fit method should have raised a different error"
...
def test_incompatible_mat_k():
     ...
     # Create test dataset of 10 observations, measured in 2 dimensions, with small sd
     ...
     try:
       ...
       kmeans_test_model = km.KMeans(k=15)
       kmeans_test_model = kmeans_test_model.fit(mat = test_clusters)

       assert False,  "Problem: no error when the number of requested clusters is more than the number of observations"
